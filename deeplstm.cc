...
int main ( int argc, char *argv[] ) {
	...
	//MatrixXi 	valid_data           = rawread ( "data/ptb/ptb.valid.txt" );
	//MatrixXi 	test_data           = rawread ( "data/ptb/ptb.test.txt" );
	
	std::string out_filename    	= "enwik8_surprisal_N" + std::to_string ( N ) +
									  "_S" + std::to_string ( S ) +
									  "_B" + std::to_string ( B ) ;
									  
	const size_t    epoch_length    = 10000;
	const size_t    test_every      = 3600;
	...
	size_t serialize_every = 12; // * test_every
	...
	dtype flops_per_iteration = count_flops ( M, N, S, B ) * ( D + 1 );
		...
		for ( size_t i = 0; i < epoch_length; i += S - 1 ) {
			...
			for ( size_t b = 0; b < B; b++ ) {
				...
				for ( size_t t = 1; t < S; t++ ) {
					...
					size_t ev_t = ( ( int * ) data.data() ) [positions[b] + t + 1];
