...
void cu_elementwise_surprisal_rlstm_forward (
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_rlstm_forward <<< num_blocks, NUM_THREADS, stream_idx>>> ( g, g2, g3, b, h, c, ct, prev_c, prev_h, rands, N,
			B );
...
void cu_elementwise_surprisal_rlstm_backward (
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_rlstm_backward <<< num_blocks, NUM_THREADS, stream_idx>>> ( dg, dg2, dg3, dh, c, ct, dc, g, g2, g3, prev_c,
...
__global__ void kernel_elementwise_surprisal_rlstm_backward (
	...
	if ( tid < elements ) {
		...
		//propagate error back through gates
		dg[o_gates + tid] = dh[tid] * ct[tid] * g[r_gates + tid];
		//dg[r_gates + tid] = //-dh[tid] * prev_h[tid] +
		...
		//dg[r_gates + tid] -= 0.00001f * ((g[r_gates + tid]) - 0);
