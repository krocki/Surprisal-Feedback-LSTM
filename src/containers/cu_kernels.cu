...
__forceinline__ __device__ dtype device_tanh_param ( dtype x, dtype k ) {
...
__forceinline__ __device__ dtype device_tanh_param_prime ( dtype x, dtype k ) {

	dtype t = x * k;

	//derivative = t * t * k?
	return ( dtype ) 1.0 - t * t;
...
/* b gives mean */
/* this modifies the variance */

__global__ void kernel_elementwise_tanh_param ( dtype* __restrict__ c, dtype* __restrict__ k, size_t n ) {

	int tid = blockDim.x * blockIdx.x + threadIdx.x;

	if ( tid < n )  c[tid] = device_tanh_param ( c[tid], k[tid] );

}

void cu_tanh_param ( dtype* __restrict__ data, dtype* __restrict__ k, size_t elements, int stream_idx ) {

	size_t num_blocks = ( elements + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_tanh_param <<< num_blocks, NUM_THREADS, stream_idx>>> ( data, k, elements );
...
void cu_elementwise_peeplstm_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ g3,
	...
	kernel_elementwise_peeplstm_forward <<< num_blocks, NUM_THREADS, stream_idx>>> ( g, g2, g3, b, h, c, ct, prev_c, N, B );
...
void cu_elementwise_surprisal_lstm_forward (
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_lstm_forward <<< num_blocks, NUM_THREADS, stream_idx>>> ( g, g2, g3, b, h, c, ct, prev_c, prev_h, rands, N,
			B );

}

void cu_elementwise_surprisal_rlstm_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ g3,
	dtype* __restrict__ G,
	...
	dtype* __restrict__ prev_h,
	dtype* __restrict__ rands,
	dtype* __restrict__ gh,
	size_t N, size_t B, int stream_idx ) {

	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_rlstm_forward <<< num_blocks, NUM_THREADS, stream_idx>>> ( g, g2, g3, G, b, h, c, ct, prev_c, prev_h, rands, gh, N,
			B );

}

void cu_elementwise_surprisal_array_lstm_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ g3,
	dtype* __restrict__ b,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_array_lstm_forward <<< num_blocks, NUM_THREADS, stream_idx>>> ( g, g2, g3, b, h, c, ct, prev_c, prev_h, rands, N, L,
			B );

}

void cu_elementwise_surprisal_array_att_lstm_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ g3,
	dtype* __restrict__ b,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_array_att_lstm_forward <<< num_blocks, NUM_THREADS, stream_idx>>> ( g, g2, g3, b, h, c, ct, prev_c, prev_h, rands, N, L,
			B );

}

void cu_elementwise_surprisal_array_stochlstm_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ g3,
	dtype* __restrict__ b,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_array_stochlstm_forward <<< num_blocks, NUM_THREADS, stream_idx>>> ( g, g2, g3, b, h, max_o, c, ct, prev_c, prev_h, rands, N, L,
			B );

}

void cu_elementwise_surprisal_lstm_base_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ b,
	dtype* __restrict__ h,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_lstm_base_forward <<< num_blocks, NUM_THREADS, stream_idx>>> ( g, g2, b, h, c, ct, prev_c, prev_h, N,
			B );

}

void cu_elementwise_surprisal_lstm_backward0 (
	dtype* __restrict__ dp,
	dtype* __restrict__ dq,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_lstm_backward0 <<< num_blocks, NUM_THREADS, stream_idx>>> ( dp, dq, q, p, x, N,
			B );

}

void cu_elementwise_surprisal_lstm_backward20 (
	dtype* __restrict__ dp,
	dtype* __restrict__ dq,
	dtype* __restrict__ q,
	dtype* __restrict__ p,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_lstm_backward20 <<< num_blocks, NUM_THREADS, stream_idx>>> ( dp, dq, q, p, x, N,
			B );

}

void cu_elementwise_surprisal_lstm_backward1 (
	dtype* __restrict__ dp,
	dtype* __restrict__ p,
	dtype* __restrict__ x,
	dtype* __restrict__ sum,
	size_t N, size_t B, int stream_idx ) {

	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_lstm_backward1 <<< num_blocks, NUM_THREADS, stream_idx>>> ( dp, p, x, sum, N,
			B );

}

void cu_elementwise_surprisal_lstm_forward0 (
	dtype* __restrict__ q,
	dtype* __restrict__ h,
	dtype* __restrict__ p,
	size_t N, size_t B, int stream_idx ) {
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_lstm_forward0 <<< num_blocks, NUM_THREADS, stream_idx>>> ( q, h, p, N,
			B );

}

void cu_elementwise_peeplstm_backward1 (
	dtype* __restrict__ prev_dc,
	dtype* __restrict__ dct,
	dtype* __restrict__ ct,
	size_t N, size_t B, int stream_idx ) {

	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_peeplstm_backward1 <<< num_blocks, NUM_THREADS, stream_idx>>> ( prev_dc, dct,
			ct, N,
			B );
...
void cu_elementwise_surprisal_lstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
	dtype* __restrict__ c,
	dtype* __restrict__ ct,
	dtype* __restrict__ dc,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_lstm_backward <<< num_blocks, NUM_THREADS, stream_idx>>> ( dg, dg2, dg3, dh, c, ct, dc, g, g2, g3, prev_c,
			prev_dc, h, prev_h, prev_dh, N, B );

}

void cu_elementwise_surprisal_rlstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
	...
	dtype* __restrict__ prev_h,
	dtype* __restrict__ prev_dh,
	dtype* __restrict__ mean_g,
	size_t N, size_t B, int stream_idx ) {


	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_rlstm_backward <<< num_blocks, NUM_THREADS, stream_idx>>> ( dg, dg2, dg3, dh, c, ct, dc, g, g2, g3, G, prev_c,
			prev_dc, h, prev_h, prev_dh, mean_g, N, B );

}

void cu_elementwise_surprisal_array_lstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_array_lstm_backward <<< num_blocks, NUM_THREADS, stream_idx>>> ( dg, dg2, dg3, dh, c, ct, dc, g, g2, g3, prev_c,
			prev_dc, h, prev_h, prev_dh, N, L, B );

}

void cu_elementwise_surprisal_array_att_lstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_array_att_lstm_backward <<< num_blocks, NUM_THREADS, stream_idx>>> ( dg, dg2, dg3, dh, c, ct, dc, g, g2, g3, prev_c,
			prev_dc, h, prev_h, prev_dh, N, L, B );

}

void cu_elementwise_surprisal_array_stochlstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_array_stochlstm_backward <<< num_blocks, NUM_THREADS, stream_idx>>> ( dg, dg2, dg3, dh, c, ct, dc, g, g2, g3, prev_c,
			prev_dc, h, max_o, prev_h, prev_dh, N, L, B );

}

void cu_elementwise_surprisal_sparselstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
	...
	dtype* __restrict__ prev_h,
	dtype* __restrict__ prev_dh,
	dtype* __restrict__ mean,
	size_t N, size_t B, size_t S, int stream_idx ) {


	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_sparselstm_backward <<< num_blocks, NUM_THREADS, stream_idx>>> ( dg, dg2, dg3, dh, c, ct, dc, g, g2, g3, prev_c,
			prev_dc, h, prev_h, prev_dh, mean, N, B, S );

}

void cu_elementwise_surprisal_lstm_base_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dh,
	dtype* __restrict__ c,
	...
	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_elementwise_surprisal_lstm_base_backward <<< num_blocks, NUM_THREADS, stream_idx>>> ( dg, dg2, dh, c, ct, dc, g, g2, prev_c,
			prev_dc, h, prev_h, prev_dh, N, B );
...
__global__ void kernel_elementwise_peeplstm_backward1 (
	dtype* __restrict__ prev_dc,
	dtype* __restrict__ dct,
	dtype* __restrict__ ct,
	size_t N, size_t B) {
	...
	if ( tid < elements ) {
		...
		prev_dc[tid] += dct[tid] * device_tanh_prime ( ct[tid] );

	}

}
__global__ void kernel_elementwise_peeplstm_forward (	dtype* __restrict__ g,
		dtype* __restrict__ g2,
		dtype* __restrict__ g3,
		dtype* __restrict__ b,
		dtype* __restrict__ h,
	...
	if ( tid < elements ) {
		...
		/* add bias */
		g[i_gates + tid] 	+= g2[i_gates + tid] + g3[i_gates + tid] + b[ ( i_gates + tid ) / B];
		g[o_gates + tid] 	+= g2[o_gates + tid] + g3[o_gates + tid] + b[ ( o_gates + tid ) / B];
		g[f_gates + tid] 	+= g2[f_gates + tid] + g3[f_gates + tid] + b[ ( f_gates + tid ) / B];
		g[c_gates + tid] 	+= g2[c_gates + tid] + g3[c_gates + tid] + b[ ( c_gates + tid ) / B];
...
__global__ void kernel_elementwise_surprisal_lstm_backward0 (
	dtype* __restrict__ dp,
	dtype* __restrict__ dq,
	dtype* __restrict__ q,
	dtype* __restrict__ p,
	...
	if ( tid < elements ) {
		...
		//q[tid] = -log2f(p[tid]) * x[tid];
		//printf("b %f\n", dq[tid % B]);
		dp[tid] = dq[tid % B] * 1 * p[tid] * x[tid];
		//printf("%f %f %f\n", p[tid], x[tid], q[tid]);

		dp[tid] = dp[tid] * (dtype)1 / p[tid];

		if (isnan(dp[tid]) || isinf(dp[tid]))
			dp[tid] = 0;
...
__global__ void kernel_elementwise_surprisal_lstm_backward20 (
	dtype* __restrict__ dp,
	dtype* __restrict__ dq,
	dtype* __restrict__ q,
	dtype* __restrict__ p,
...
__global__ void kernel_elementwise_surprisal_lstm_backward1 (
	dtype* __restrict__ dp,
	dtype* __restrict__ p,
	dtype* __restrict__ x,
	dtype* __restrict__ sums,
	size_t N, size_t B ) {
...
__global__ void kernel_elementwise_surprisal_lstm_forward0 (
	dtype* __restrict__ q,
	dtype* __restrict__ p,
	dtype* __restrict__ x,
	size_t N, size_t B ) {
	...
	if ( tid < elements ) {
		...
		q[tid] = log(p[tid]) * x[tid];

		//q[tid] = -log(q[tid]);

		if (isnan(q[tid]) || isinf(q[tid]))
			q[tid] = 0.00f;
...
__global__ void kernel_elementwise_surprisal_lstm_base_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ b,
	dtype* __restrict__ h,
	...
	if ( tid < elements ) {
		...
		//printf("%f %f %f %f\n", g3[i_gates + tid], g3[o_gates + tid], g3[f_gates + tid], g3[c_gates + tid]);
		...
		c[tid] 				= 	((dtype)1 - g[f_gates + tid]) * prev_c[tid];
		c[tid] 				+= 	g[i_gates + tid] * g[c_gates + tid];
...
__global__ void kernel_elementwise_surprisal_lstm_forward (
	...
	if ( tid < elements ) {
		...
		//float stoch_g = g[o_gates + tid] > rands[o_gates + tid];
...
#define f_gates 2 * elements
#define c_gates 3 * elements
#define r_gates 4 * elements

/* backup

__global__ void kernel_elementwise_surprisal_rlstm_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ g3,
	dtype* __restrict__ b,
	...
	if ( tid < elements ) {
		...
		g[f_gates + tid] 	+= g2[f_gates + tid] + g3[f_gates + tid] + b[ ( f_gates + tid ) / B];
		g[c_gates + tid] 	+= g2[c_gates + tid] + g3[c_gates + tid] + b[ ( c_gates + tid ) / B];
		g[r_gates + tid] 	+= g2[r_gates + tid] + g3[r_gates + tid] + b[ ( r_gates + tid ) / B];

		// g[i_gates + tid] 	+= g2[i_gates + tid] + b[ ( i_gates + tid ) / B];
		// g[o_gates + tid] 	+= g2[o_gates + tid] + b[ ( o_gates + tid ) / B];
		// g[f_gates + tid] 	+= g2[f_gates + tid] + b[ ( f_gates + tid ) / B];
		// g[c_gates + tid] 	+= g2[c_gates + tid] + b[ ( c_gates + tid ) / B];
		// g[r_gates + tid] 	+= g3[r_gates + tid] + b[ ( r_gates + tid ) / B];


		g[i_gates + tid] 	= 	device_logistic	( g[i_gates + tid] );
		g[o_gates + tid] 	= 	device_logistic	( g[o_gates + tid] );
		g[f_gates + tid] 	= 	device_logistic	( g[f_gates + tid] );
		g[r_gates + tid] 	= 	device_logistic	( g[r_gates + tid] );
		g[c_gates + tid] 	= 	device_tanh	( g[c_gates + tid] );

		c[tid] 				= 	((dtype)1 - g[f_gates + tid] * g[r_gates + tid]) * prev_c[tid];
		c[tid] 				+= 	g[r_gates + tid] * g[i_gates + tid] * g[c_gates + tid];

		ct[tid] 			= 	device_tanh ( c[tid] );

		//float stoch_g = g[o_gates + tid] > rands[o_gates + tid];

		h[tid] 				= 	g[r_gates + tid] * g[o_gates + tid] * ct[tid];
		h[tid]				+= 	((dtype)1 - g[r_gates + tid]) * prev_h[tid];
...
__global__ void kernel_elementwise_surprisal_rlstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
	...
	if ( tid < elements ) {
		...
		dc[tid] = dc[tid] + dh[tid] * g[r_gates + tid] * g[o_gates + tid] * device_tanh_prime ( ct[tid] );

		//prev_dh[tid] += dh[tid];
		prev_dc[tid] += dc[tid] * ((dtype)1 - g[f_gates + tid] * g[r_gates + tid]);
		prev_dh[tid] += dh[tid] * ((dtype)1 - g[r_gates + tid]);

		//fix
		//dc[tid] = dc[tid] * device_tanh_prime ( ct[tid] );

		//propagate error back through gates
		dg[o_gates + tid] = dh[tid] * ct[tid] * g[r_gates + tid];
		dg[r_gates + tid] = -dh[tid] * prev_h[tid] + g[o_gates + tid] * ct[tid] * dh[tid] + g[i_gates + tid] * g[c_gates + tid] * dc[tid] - g[f_gates + tid] * prev_c[tid] * dc[tid];
		dg[i_gates + tid] = dc[tid] * g[c_gates + tid] * g[r_gates + tid];
		dg[f_gates + tid] = -dc[tid] * prev_c[tid] * g[r_gates + tid];
		dg[c_gates + tid] = dc[tid] * g[i_gates + tid] * g[r_gates + tid];

		//sparsity
		// if (tid == 0) {

		// 	printf("%d, o %f i %f f %f c %f r %f --- o %f i %f f %f c %f r %f\n",
		// 		   tid,
		// 		   mean_g[o_gates + tid],
		// 		   mean_g[i_gates + tid],
		// 		   mean_g[f_gates + tid],
		...
		//dg[r_gates + tid] -= 0.00001f * (0 - (g[r_gates + tid]));

		//propagate error back through gates, nonlinearities
		dg[i_gates + tid] 	= 	dg[i_gates + tid] * device_logistic_prime	( g[i_gates + tid] );
		dg[o_gates + tid] 	= 	dg[o_gates + tid] * device_logistic_prime	( g[o_gates + tid] );
		dg[f_gates + tid] 	= 	dg[f_gates + tid] * device_logistic_prime	( g[f_gates + tid] );
		dg[r_gates + tid] 	= 	dg[r_gates + tid] * device_logistic_prime	( g[r_gates + tid] );
		dg[c_gates + tid] 	= 	dg[c_gates + tid] * device_tanh_prime	( g[c_gates + tid] );

		dg3[i_gates + tid] = dg[i_gates + tid];
		dg3[o_gates + tid] = dg[o_gates + tid];
		dg3[f_gates + tid] = dg[f_gates + tid];
		dg3[r_gates + tid] = dg[r_gates + tid];
		dg3[c_gates + tid] = dg[c_gates + tid];

		dg2[i_gates + tid] = dg[i_gates + tid];
		dg2[o_gates + tid] = dg[o_gates + tid];
		dg2[f_gates + tid] = dg[f_gates + tid];
		dg2[r_gates + tid] = dg[r_gates + tid];
		dg2[c_gates + tid] = dg[c_gates + tid];
...
__global__ void kernel_elementwise_surprisal_rlstm_forward (
	...
	if ( tid < elements ) {
		...
		//keep prob
		//G[tid] 	= 	rands[tid] <= (0.9f + fabsf(gh[tid]));
		//1. 
		G[tid] 	= 	rands[tid] <= (0.2f + fabsf(gh[tid]));
		//2. 
		//G[tid] 	= 	rands[tid] <= (expf (fabsf(gh[tid] * 3.0f)) - 1.0f);
		//3.
		//G[tid] 	= 	rands[tid] <= (1.0f -  1.0f/(fabsf(gh[tid]) + 2.0f));
		//printf("tid %d gh %f\n", tid, fabsf(gh[tid]));

		//G[r_gates + tid] 	=   g[r_gates + tid];

		g[c_gates + tid] 	= 	device_tanh	( g[c_gates + tid] );

		c[tid] 				= 	((dtype)1 - g[f_gates + tid] * G[tid]) * prev_c[tid];
		c[tid] 				+= 	G[tid] * g[i_gates + tid] * g[c_gates + tid];

		ct[tid] 			= 	device_tanh ( c[tid] );

		//float stoch_g = g[o_gates + tid] > rands[o_gates + tid];

		h[tid] 				= 	g[o_gates + tid] * ct[tid] * G[tid];
		//h[tid]				+= 	((dtype)1 - g[r_gates + tid]) * prev_h[tid];
...
__global__ void kernel_elementwise_surprisal_rlstm_backward (
	...
	if ( tid < elements ) {
		...
		//dh[tid] = dh[tid] * device_tanh_prime	( h[tid] );
		dc[tid] = dc[tid] + dh[tid] * g[o_gates + tid] * device_tanh_prime ( ct[tid] ) * G[tid] ;

		//prev_dh[tid] += dh[tid];
		prev_dc[tid] += dc[tid] * ((dtype)1 - g[f_gates + tid] * G[tid]);
		//prev_dh[tid] += dh[tid] * ((dtype)1 - g[r_gates + tid]);
		...
		//propagate error back through gates
		dg[o_gates + tid] = dh[tid] * ct[tid] * G[tid];
		dg[i_gates + tid] = dc[tid] * g[c_gates + tid] * G[tid];
		dg[f_gates + tid] = -dc[tid] * prev_c[tid] * G[tid];
		dg[c_gates + tid] = dc[tid] * g[i_gates + tid] * G[tid];
		...
		//dg[r_gates + tid] -= 0.001f * ((mean_g[r_gates + tid] - 0.0f));
...
// 		h[tid] 				= 	g[o_gates + tid] * ct[tid] * G[r_gates + tid];
// 		//h[tid]				+= 	((dtype)1 - g[r_gates + tid]) * prev_h[tid];
...
// 		//dh[tid] = dh[tid] * device_tanh_prime	( h[tid] );
// 		dc[tid] = dc[tid] + dh[tid] * g[o_gates + tid] * G[r_gates + tid] * device_tanh_prime ( ct[tid] );
...
// 		//propagate error back through gates
// 		dg[o_gates + tid] = dh[tid] * ct[tid] * G[r_gates + tid];
// 		//dg[r_gates + tid] = dh[tid] * g[o_gates + tid] * ct[tid] +
// 		//g[i_gates + tid] * g[c_gates + tid] * dc[tid] - g[f_gates + tid] * prev_c[tid] * dc[tid];
// 		dg[i_gates + tid] = dc[tid] * g[c_gates + tid] * G[r_gates + tid];
// 		dg[f_gates + tid] = -dc[tid] * prev_c[tid] * G[r_gates + tid];
// 		dg[c_gates + tid] = dc[tid] * g[i_gates + tid] * G[r_gates + tid];
...
__global__ void kernel_elementwise_surprisal_lstm_base_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dh,
	dtype* __restrict__ c,
	...
	if ( tid < elements ) {
		...
		dc[tid] = dc[tid] + dh[tid] * g[o_gates + tid] * device_tanh_prime ( ct[tid] );
		//prev_dh[tid] += dh[tid];
		prev_dc[tid] += dc[tid] * ((dtype)1 - g[f_gates + tid]);
...
__global__ void kernel_elementwise_surprisal_sparselstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
	...
	if ( tid < elements ) {
		...
		// 	printf("%d, o %f i %f f %f c %f --- o %f i %f f %f c %f \n",
		// 		   tid,
		// 		   mean_g[o_gates + tid],
		// 		   mean_g[i_gates + tid],
		// 		   mean_g[f_gates + tid],
		...
		//dg[i_gates + tid] -= 0.05f * (0.05f - (mean_g[i_gates + tid]));
		dg[f_gates + tid] -= 0.000001f * (0.00f - (mean_g[f_gates + tid]));
		//dg[o_gates + tid] -= 0.0001f * (0.1f - (mean_g[o_gates + tid]));
...
// 		// g3[f_gates + tid] 	= 	device_tanh	( g3[f_gates + tid] );
// 		// g3[i_gates + tid] 	= 	device_tanh	( g3[i_gates + tid] );
// 		// g3[o_gates + tid] 	= 	device_tanh	( g3[o_gates + tid] );
// 		// g3[c_gates + tid] 	= 	device_tanh	( g3[c_gates + tid] );
...
// 		// dg3[i_gates + tid] = dg[i_gates + tid] * (g[i_gates + tid]);
// 		// dg3[o_gates + tid] = dg[o_gates + tid] * (g[o_gates + tid]);
// 		// dg3[f_gates + tid]  = -dc[tid] * (g[f_gates + tid]) * prev_c[tid];
// 		// //dg3[f_gates + tid] 	= dg3[f_gates + tid] * device_logistic_prime ( g3[f_gates + tid] );
// 		// dg3[i_gates + tid]  = dc[tid] * g[i_gates + tid] * g[c_gates + tid] * g3[c_gates + tid];
// 		// //dg3[i_gates + tid] 	= dg3[i_gates + tid] * device_logistic_prime ( g3[i_gates + tid] );
// 		// dg3[o_gates + tid]  = dh[tid] * ct[tid] * g[o_gates + tid];
// 		// //dg3[o_gates + tid] 	= dg3[o_gates + tid] * device_logistic_prime ( g3[o_gates + tid] );
// 		// dg3[c_gates + tid] = dg[c_gates + tid] * (g[c_gates + tid]) * (g[i_gates + tid]) * (g3[i_gates + tid]);
// 		//dg3[c_gates + tid] 	= dg3[c_gates + tid] * device_logistic_prime ( g3[c_gates + tid] );

// 		// dg[i_gates + tid] = dg[i_gates + tid] * (g3[i_gates + tid]);
// 		// dg[o_gates + tid] = dg[o_gates + tid] * (g3[o_gates + tid]);
// 		// dg[f_gates + tid] = dg[f_gates + tid] * (g3[f_gates + tid]);
// 		// dg[c_gates + tid] = dg[c_gates + tid] * (g3[c_gates + tid]);
...
// 		// dg3[i_gates + tid] = dg[i_gates + tid] * (g2[i_gates + tid]);
// 		// dg3[o_gates + tid] = dg[o_gates + tid] * (g2[o_gates + tid]);
// 		// dg3[f_gates + tid] = dg[f_gates + tid] * (g2[f_gates + tid]);
// 		// dg3[c_gates + tid] = dg[c_gates + tid] * (g2[c_gates + tid]);
...
#define f_gates 2 * N * B
#define c_gates 3 * N * B
#define r_gates 4 * N * B

void cu_accumulate_g_activity (
	dtype* __restrict__ g,
	dtype* __restrict__ mean_g,
	size_t N, size_t B, size_t S ) {

	size_t num_blocks = ( N * B + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_accumulate_g_activity <<< num_blocks, NUM_THREADS, 0>>> ( g, mean_g, N, B, S );

}

__global__ void kernel_accumulate_g_activity (
	dtype* __restrict__ g,
	dtype* __restrict__ mean_g,
	size_t N, size_t B, size_t S ) {
	...
	if ( tid < elements ) {
		...
		mean_g[( o_gates + tid )] = mean_g[( o_gates + tid )] * 0.999f + 0.001f * g[( o_gates + tid ) ];
		mean_g[( i_gates + tid )] = mean_g[( i_gates + tid )] * 0.999f + 0.001f * g[( i_gates + tid ) ];
		mean_g[( f_gates + tid )] = mean_g[( f_gates + tid )] * 0.999f + 0.001f * g[( f_gates + tid ) ];
		mean_g[( c_gates + tid )] = mean_g[( c_gates + tid )] * 0.999f + 0.001f * g[( c_gates + tid ) ];
		//mean_g[( r_gates + tid )] = mean_g[( r_gates + tid )] * 0.999f + 0.001f * g[( r_gates + tid ) ];
...
#define f_gates 2 * N * B * L
#define c_gates 3 * N * B * L
#define r_gates 4 * N * B * L

__global__ void kernel_elementwise_surprisal_array_lstm_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ g3,
	dtype* __restrict__ b,
...
#pragma unroll
		...
		for ( int l = 0; l < L; l++ ) {
			...
			/* add bias */
			g[i_gates + ltid] 	+= g2[i_gates + ltid] + g3[i_gates + ltid] + b[ ( i_gates + ltid ) / B];
			g[o_gates + ltid] 	+= g2[o_gates + ltid] + g3[o_gates + ltid] + b[ ( o_gates + ltid ) / B];
			g[f_gates + ltid] 	+= g2[f_gates + ltid] + g3[f_gates + ltid] + b[ ( f_gates + ltid ) / B];
			g[c_gates + ltid] 	+= g2[c_gates + ltid] + g3[c_gates + ltid] + b[ ( c_gates + ltid ) / B];
...
__global__ void kernel_elementwise_surprisal_array_lstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
...
#pragma unroll
		...
		for ( int l = 0; l < L; l++ ) {
			...
			dg3[i_gates + ltid] = dg[i_gates + ltid];
			dg3[o_gates + ltid] = dg[o_gates + ltid];
			dg3[f_gates + ltid] = dg[f_gates + ltid];
			dg3[c_gates + ltid] = dg[c_gates + ltid];

			dg2[i_gates + ltid] = dg[i_gates + ltid];
			dg2[o_gates + ltid] = dg[o_gates + ltid];
			dg2[f_gates + ltid] = dg[f_gates + ltid];
			dg2[c_gates + ltid] = dg[c_gates + ltid];
...
__global__ void kernel_elementwise_surprisal_array_att_lstm_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ g3,
	dtype* __restrict__ b,
...
#pragma unroll
		...
		for ( int l = 0; l < L; l++ ) {
			...
			g[f_gates + ltid] 	+= g2[f_gates + ltid] + g3[f_gates + ltid] + b[ ( f_gates + ltid ) / B];
			g[c_gates + ltid] 	+= g2[c_gates + ltid] + g3[c_gates + ltid] + b[ ( c_gates + ltid ) / B];
			g[r_gates + ltid] 	+= g3[r_gates + ltid] + b[ ( r_gates + ltid ) / B];
			...
			//for softmax
			total_prob += device_exp ( g[r_gates + ltid] );
		...
		for ( int l = 0; l < L; l++ ) {
			...
			//softmax
			g[r_gates + ltid] = device_exp ( g[r_gates + ltid] ) / total_prob;

			c[ltid] 				= 	((dtype)1 - g[f_gates + ltid] * g[r_gates + ltid]) * prev_c[ltid];
			c[ltid] 				+= 	g[i_gates + ltid] * g[c_gates + ltid] * g[r_gates + ltid];

			ct[ltid] 			= 	device_tanh ( c[ltid] );

			//float stoch_g = g[o_gates + tid] > rands[o_gates + tid];

			h[tid] 				+= 	g[o_gates + ltid] * ct[ltid] * g[r_gates + ltid];
...
__global__ void kernel_elementwise_surprisal_array_att_lstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
...
#pragma unroll
		...
		for ( int l = 0; l < L; l++ ) {
			...
			dc[ltid] = dc[ltid] + dh[tid] * g[o_gates + ltid] * g[r_gates + ltid] * device_tanh_prime ( ct[ltid] );
			prev_dc[ltid] += dc[ltid] * ((dtype)1 - g[f_gates + ltid] * g[r_gates + ltid]);

			//propagate error back through gates
			dg[o_gates + ltid] = dh[tid] * ct[ltid] * g[r_gates + ltid];
			dg[i_gates + ltid] = dc[ltid] * g[c_gates + ltid] * g[r_gates + ltid]; 
			dg[f_gates + ltid] = -dc[ltid] * prev_c[ltid] * g[r_gates + ltid];
			dg[c_gates + ltid] = dc[ltid] * g[i_gates + ltid] * g[r_gates + ltid];

			dg[r_gates + ltid] = dc[ltid] * g[i_gates + ltid] * g[c_gates + ltid] +
								 dh[tid] * ct[ltid] * g[o_gates + ltid] +
								 -dc[ltid] * prev_c[ltid] * g[f_gates + ltid];
			...
			//softmax derivative
			dg[r_gates + ltid] = dg[r_gates + ltid] * ( g[r_gates + ltid] );
			total_prob += dg[r_gates + ltid];
		}
		...
		for ( int l = 0; l < L; l++ ) {
			...
			int ltid = tid + l * N * B;
			dg[r_gates + ltid] -= g[r_gates + ltid] * total_prob;
		...
		for ( int l = 0; l < L; l++ ) {
			...
			dg3[f_gates + ltid] = dg[f_gates + ltid];
			dg3[c_gates + ltid] = dg[c_gates + ltid];
			dg3[r_gates + ltid] = dg[r_gates + ltid];
...
__global__ void kernel_elementwise_surprisal_array_stochlstm_forward (
	dtype* __restrict__ g,
	dtype* __restrict__ g2,
	dtype* __restrict__ g3,
	dtype* __restrict__ b,
...
//		max_o_idx = tid ;
		...
		/* add bias */
		g[i_gates + max_o_idx] 	+= g2[i_gates + max_o_idx] + g3[i_gates + max_o_idx] + b[ ( i_gates + max_o_idx ) / B];
		g[o_gates + max_o_idx] 	+= g2[o_gates + max_o_idx] + g3[o_gates + max_o_idx] + b[ ( o_gates + max_o_idx ) / B];
		g[f_gates + max_o_idx] 	+= g2[f_gates + max_o_idx] + g3[f_gates + max_o_idx] + b[ ( f_gates + max_o_idx ) / B];
		g[c_gates + max_o_idx] 	+= g2[c_gates + max_o_idx] + g3[c_gates + max_o_idx] + b[ ( c_gates + max_o_idx ) / B];
...
__global__ void kernel_elementwise_surprisal_array_stochlstm_backward (
	dtype* __restrict__ dg,
	dtype* __restrict__ dg2,
	dtype* __restrict__ dg3,
	dtype* __restrict__ dh,
	...
	if ( tid < elements ) {
		...
		for ( int l = 0; l < L; l++ ) {
			...
			if ( ltid2 != ltid )
				...
				prev_dc[ltid2] = dc[ltid2] * ((dtype)1 - g[f_gates + ltid2]);
...
void cu_decay (
	dtype* __restrict__ p,
	dtype* __restrict__ d,
	dtype decay, size_t elements ) {

	size_t num_blocks = ( elements + NUM_THREADS - 1 ) / NUM_THREADS;
	kernel_decay <<< num_blocks, NUM_THREADS, 0>>> ( p, d, decay, elements );

}

__global__ void kernel_decay (
	dtype* __restrict__ p,
	dtype* __restrict__ d,
	dtype decay, size_t elements ) {
	...
	if ( tid < elements ) {
		...
		// if (tid == 0)
		// printf("1 %d %.8f %.8f %.8f\n", tid, p[tid], d[tid], d[tid] - p[tid] * decay );

		d[tid] += p[tid] * decay;

		// 	if (tid == 1000)
		// printf("2 %d %.8f %.8f\n", tid, p[tid] * decay, d[tid] );
	}
