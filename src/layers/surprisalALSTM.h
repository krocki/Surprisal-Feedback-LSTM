...
*
*   Implementation of surprisalArrayLSTM layer
*
*/

#ifndef _surprisalArrayLSTM_H_
#define _surprisalArrayLSTM_H_
...
template <typename T>
class surprisalArrayLSTM : public Timelayer<T> {
...
public:
	...
	/* main constructor */
	surprisalArrayLSTM ( size_t _M, size_t _N, size_t _B,
						 size_t _S, size_t _L = 1 ) : L ( _L ),

		Timelayer<T> ( _M, _N, _B, _S,

	{   "surprisal_array_lstm"        },
...
#define o_gates 1 * N * s(t, g).rows()
	...
	virtual void forward ( bool apply_dropout, size_t t = 1 ) {
		...
		//fused
		cu_elementwise_surprisal_array_lstm_forward ( & ( s ( t, g ).cu_data[0] ),
				& ( s ( t, g2 ).cu_data[0] ),
				& ( s ( t, g3 ).cu_data[0] ),
				& ( p ( b ).cu_data[0] ),
				& ( s ( t, h ).cu_data[0] ),
	...
	virtual void backward ( bool apply_dropout, size_t t ) {
		...
		cu_elementwise_surprisal_array_lstm_backward ( & ( g ( t, g ).cu_data[0] ), & ( g ( t, g2 ).cu_data[0] ), & ( g ( t,
				g3 ).cu_data[0] ),
				& ( g ( t, h ).cu_data[0] ),
				& ( s ( t, c ).cu_data[0] ),
				& ( s ( t, ct ).cu_data[0] ),
	...
	/* optional */
	/* copy constr */
	//surprisalArrayLSTM( const surprisalArrayLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }

	/* assignent operator */
	//surprisalArrayLSTM& operator=( const surprisalArrayLSTM& src) { Timelayer::operator=(src); return *this; }
