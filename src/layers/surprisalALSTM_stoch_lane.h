...
*
*   Implementation of surprisalArrayStochLSTM layer
*
*/

#ifndef _surprisalArrayStochLSTM_H_
#define _surprisalArrayStochLSTM_H_
...
template <typename T>
class surprisalArrayStochLSTM : public Timelayer<T> {
...
public:
	...
	/* main constructor */
	surprisalArrayStochLSTM ( size_t _M, size_t _N, size_t _B,
							  size_t _S, size_t _L = 1 ) : L ( _L ),

		Timelayer<T> ( _M, _N, _B, _S,

	{   "surprisal_array_stoch_lane_lstm"        },

	{
		...
		std::cout << "stoch array lstm levels :" << _L << std::endl;
...
#define o_gates 1 * N * s(t, g).rows()
	...
	virtual void forward ( bool apply_dropout, size_t t = 1 ) {
		...
		//fused
		cu_elementwise_surprisal_array_stochlstm_forward ( & ( s ( t, g ).cu_data[0] ),
				& ( s ( t, g2 ).cu_data[0] ),
				& ( s ( t, g3 ).cu_data[0] ),
				& ( p ( b ).cu_data[0] ),
				& ( s ( t, h ).cu_data[0] ),
	...
	virtual void backward ( bool apply_dropout, size_t t ) {
		...
		cu_elementwise_surprisal_array_stochlstm_backward ( & ( g ( t, g ).cu_data[0] ), & ( g ( t, g2 ).cu_data[0] ), & ( g ( t,
				g3 ).cu_data[0] ),
				& ( g ( t, h ).cu_data[0] ),
				& ( s ( t, c ).cu_data[0] ),
				& ( s ( t, ct ).cu_data[0] ),
	...
	/* optional */
	/* copy constr */
	//surprisalArrayStochLSTM( const surprisalArrayStochLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }

	/* assignent operator */
	//surprisalArrayStochLSTM& operator=( const surprisalArrayStochLSTM& src) { Timelayer::operator=(src); return *this; }
...
#endif /* _surprisalArrayStochLSTM_H_ */