...
*
*   Implementation of surprisalLSTMBASE layer
*
*/

#ifndef _surprisalLSTMBASE_H_
#define _surprisalLSTMBASE_H_
...
template <typename T>
class surprisalLSTMBASE : public Timelayer<T> {
...
public:
	...
	/* main constructor */
	surprisalLSTMBASE ( size_t _M, size_t _N, size_t _B,
						size_t _S ) :
...
#define o_gates 1 * N * s(t, g).rows()
	...
	virtual void forward ( bool apply_dropout, size_t t = 1 ) {
		...
		cu_elementwise_surprisal_lstm_forward0 ( s ( t, q ).cu_data, s ( t - 1, p ).cu_data, s ( t, x ).cu_data, this->M,
				s ( t,
					q ).rows() );
		...
		// cu_randn ( rands.cu_data, rands.size(), 0, 0.01 );
		...
		//fused
		cu_elementwise_surprisal_lstm_base_forward ( & ( s ( t, g ).cu_data[0] ),
				& ( s ( t, g2 ).cu_data[0] ),
				& ( p ( b ).cu_data[0] ),
				& ( s ( t, h ).cu_data[0] ),
				& ( s ( t, c ).cu_data[0] ),
	...
	virtual void backward ( bool apply_dropout, size_t t ) {
		...
		cu_elementwise_surprisal_lstm_base_backward ( & ( g ( t, g ).cu_data[0] ), & ( g ( t, g2 ).cu_data[0] ),
				& ( g ( t, h ).cu_data[0] ),
				& ( s ( t, c ).cu_data[0] ),
				& ( s ( t, ct ).cu_data[0] ),
				& ( g ( t, c ).cu_data[0] ),
	...
	/* optional */
	/* copy constr */
	//surprisalLSTMBASE( const surprisalLSTMBASE& src) : Timelayer(src.M, src.N, src.B, src.S)  { }

	/* assignent operator */
	//surprisalLSTMBASE& operator=( const surprisalLSTMBASE& src) { Timelayer::operator=(src); return *this; }
