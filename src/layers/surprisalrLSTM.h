...
class surprisalrLSTM : public Timelayer<T> {
	...
	public:
		...
		
		{
			...
			std::make_tuple ( "g", _B, 5 * _N ),
			std::make_tuple ( "g2", _B, 5 * _N ),
			std::make_tuple ( "g3", _B, 5 * _N ),
			std::make_tuple ( "G", _B, 5 * _N ),
			...
			/* define params */
			std::make_tuple ( "W", _M, 5 * _N ),
			std::make_tuple ( "U", _N, 5 * _N ),
			std::make_tuple ( "V", 1, 5 * _N ),
			std::make_tuple ( "b", 1, 5 * _N ),
			...
			//r bias
			p ( b ).block_forall ( 4 * N, N, [ = ] () { return 5; } );
