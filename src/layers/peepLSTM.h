...
*
*   Implementation of peepLSTM layer
*
*/
...
template <typename T>
class peepLSTM : public Timelayer<T> {
...
public:
	...
	/* main constructor */
	peepLSTM ( size_t _M, size_t _N, size_t _B,
			   size_t _S ) :
...
#define o_gates 1 * N * s(t, g).rows()
	...
	virtual void forward ( bool apply_dropout, size_t t = 1 ) {
		...
		CU_GEMM ( s ( t, g3 ), s ( t - 1, ct ), p ( V ), false, false, 1, 0 );
		...
		//fused
		cu_elementwise_peeplstm_forward (	& ( s ( t, g ).cu_data[0] ),
											& ( s ( t, g2 ).cu_data[0] ),
											& ( s ( t, g3 ).cu_data[0] ),
											& ( p ( b ).cu_data[0] ),
											& ( s ( t, h ).cu_data[0] ),
	...
	virtual void backward ( bool apply_dropout, size_t t ) {
		...
		CU_GEMM ( d ( V ), s ( t - 1, ct ), g ( t, g ), true, false );
		...
		//carry - c state
		CU_GEMM ( g ( t, ct ), g ( t, g ), p ( V ), false , true, 1, 0 );

		cu_elementwise_peeplstm_backward1( g ( t - 1, c).cu_data,  g ( t, ct ).cu_data, s ( t - 1, ct).cu_data, N, g ( t, c ).rows() );

		//dg[c_gates + tid] * device_tanh_prime	( g[c_gates + tid] );
	...
	virtual void reset ( dtype std ) {
		...
		randn ( s ( 0, ct ), ( dtype ) 0, ( dtype ) std );
		// s ( 0, h ).cu_zero();
		// s ( 0, c ).cu_zero();
	}
	...
	/* optional */
	/* copy constr */
	//peepLSTM( const peepLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }

	/* assignent operator */
	//peepLSTM& operator=( const peepLSTM& src) { Timelayer::operator=(src); return *this; }
