...
*
*   Implementation of surprisal_sparse_lstm layer
*
*/

#ifndef _surprisal_sparse_lstm_H_
#define _surprisal_sparse_lstm_H_
...
template <typename T>
class surprisal_sparse_lstm : public Timelayer<T> {
...
public:
	...
	/* main constructor */
	surprisal_sparse_lstm ( size_t _M, size_t _N, size_t _B,
							size_t _S ) :

		Timelayer<T> ( _M, _N, _B, _S,

	{   "surprisal_sparselstm"        },
...
#define o_gates 1 * N * s(t, g).rows()
	...
	virtual void backward ( bool apply_dropout, size_t t ) {
		...
		cu_accumulate_g_activity (
			& ( s ( t, g ).cu_data[0] ),
			& ( ( mean_g ) .cu_data[0] ),
			N, g ( t, c ).rows(), this->S );

		cu_elementwise_surprisal_sparselstm_backward ( & ( g ( t, g ).cu_data[0] ), & ( g ( t, g2 ).cu_data[0] ), & ( g ( t,
				g3 ).cu_data[0] ),
				& ( g ( t, h ).cu_data[0] ),
				& ( s ( t, c ).cu_data[0] ),
				& ( s ( t, ct ).cu_data[0] ),
		...
		//cu_decay ( & ( p ( U ).cu_data[0] ), & ( d ( U ).cu_data[0] ), 0.01f, p ( U ).cols() * p ( U ).rows() );
		//cu_decay ( & ( p ( V ).cu_data[0] ), & ( d ( V ).cu_data[0] ), 0.0001f, p ( V ).cols() * p ( V ).rows() );
		//cu_decay ( & ( p ( W ).cu_data[0] ), & ( d ( W ).cu_data[0] ), 0.0001f, p ( W ).cols() * p ( W ).rows() );
	...
	/* optional */
	/* copy constr */
	//surprisal_sparse_lstm( const surprisal_sparse_lstm& src) : Timelayer(src.M, src.N, src.B, src.S)  { }

	/* assignent operator */
	//surprisal_sparse_lstm& operator=( const surprisal_sparse_lstm& src) { Timelayer::operator=(src); return *this; }
...
#endif /* _surprisal_sparse_lstm_H_ */