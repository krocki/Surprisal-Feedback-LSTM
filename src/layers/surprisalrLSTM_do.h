...
*
*   Implementation of surprisalrLSTM layer
*
*/

#ifndef _surprisalrLSTM_H_
#define _surprisalrLSTM_H_
...
template <typename T>
class surprisalrLSTM : public Timelayer<T> {
...
public:
	...
	T mean_g;
	/* main constructor */
	surprisalrLSTM ( size_t _M, size_t _N, size_t _B,
					 size_t _S ) :
...
#define o_gates 1 * N * s(t, g).rows()
	...
	virtual void forward ( bool apply_dropout, size_t t = 1 ) {
		...
		cu_sub ( & ( g ( t, p ).cu_data[0] ), & ( s ( t - 1, p ).cu_data[0] ), & ( s ( t, x ).cu_data[0] ), this->M * s ( t - 1,
				 p ).rows() );

		// propagate through linear layer h->y
		// cublasSetStream ( handle, streams[1] );
		// CU_GEMM ( d ( Wy ), s ( t - 1, h ), g ( t, p ), true, false );
		...
		//g(t) .* ht-1?
		// cu_elementwise_mult ( & ( g ( t, h ) ).cu_data[0], & ( g ( t, h ) ).cu_data[0],
		// 					  & ( s ( t - 1, h ) ).cu_data[0], this->M * s ( t - 1, p ).rows() );
		// sync_stream ( 1 );
		// sync_stream ( 2 );
		...
		//fused
		cu_elementwise_surprisal_rlstm_forward ( & ( s ( t, g ).cu_data[0] ),
				& ( s ( t, g2 ).cu_data[0] ),
				& ( s ( t, g3 ).cu_data[0] ),
				& ( s ( t, G ).cu_data[0] ),
				& ( p ( b ).cu_data[0] ),
	...
	virtual void backward ( bool apply_dropout, size_t t ) {
		...
		cu_elementwise_surprisal_rlstm_backward ( & ( g ( t, g ).cu_data[0] ), & ( g ( t, g2 ).cu_data[0] ), & ( g ( t,
				g3 ).cu_data[0] ),
				& ( g ( t, h ).cu_data[0] ),
				& ( s ( t, c ).cu_data[0] ),
				& ( s ( t, ct ).cu_data[0] ),
	...
	/* optional */
	/* copy constr */
	//surprisalrLSTM( const surprisalrLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }

	/* assignent operator */
	//surprisalrLSTM& operator=( const surprisalrLSTM& src) { Timelayer::operator=(src); return *this; }
