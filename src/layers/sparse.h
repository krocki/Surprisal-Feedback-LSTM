...
*
*   Implementation of sparseLSTM layer
*
*/

#ifndef _sparseLSTM_H_
#define _sparseLSTM_H_
...
template <typename T>
class sparseLSTM : public Timelayer<T> {
	...
	public:
		...
		/* main constructor */
		sparseLSTM ( size_t _M, size_t _N, size_t _B,
		...
		
		{
			...
			std::cout << "sparselstm levels :" << _L << std::endl;
			/*init*/
			matrix_init ( p ( W ) );
			matrix_init ( p ( U ) );
...
#define o_gates 1 * N * L * s(t, g).rows()
		...
		virtual void forward ( bool apply_dropout, size_t t = 1 ) {
			...
			//fused
			cu_elementwise_sparselstm_forward (	& ( s ( t, g ).cu_data[0] ),
												& ( s ( t, g2 ).cu_data[0] ),
												& ( p ( b ).cu_data[0] ),
												& ( s ( t, h ).cu_data[0] ),
												& ( s ( t, c ).cu_data[0] ),
		...
		virtual void backward ( bool apply_dropout, size_t t ) {
			...
			cu_elementwise_sparselstm_backward (	& ( g ( t, g ).cu_data[0] ),
													& ( g ( t, y ).cu_data[0] ),
													& ( s ( t, c ).cu_data[0] ),
													& ( s ( t, ct ).cu_data[0] ),
													& ( g ( t, c ).cu_data[0] ),
			...
			// if ( t == 1 )
			// 	cu_elementwise_sparselstm_sparsity ( N, L, g ( t, c ).rows(), S, & ( p ( b ).cu_data[0] ) );
		...
		/* optional */
		/* copy constr */
		//sparseLSTM( const sparseLSTM& src) : Timelayer(src.M, src.N, src.B, src.S)  { }
		
		/* assignent operator */
		//sparseLSTM& operator=( const sparseLSTM& src) { Timelayer::operator=(src); return *this; }
